...
uint findAllClusters(Board* board){
    //printf("initializing search for all clusters..\n");
    int tilesInCluster;
    int color;
    uint id, biggestCluster = 0, biggestId = 0;

    uint *ret = (uint *) malloc(2*sizeof(uint));
...
void removeCluster(Board *board, uint line, uint column) {

    uint id = board->clusterSets[convert(line, column, board->columns)];

    for (uint line = 1; line <= board->lines; line++) {
        for (uint column = 1; column <= board->columns; column++) {
            if (board->clusterSets[convert(line, column, board->columns)] == id) board->tiles[line - 1][column - 1] = -1;
        }
    }
}
