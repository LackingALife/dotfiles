...
VectorList *addToVectorList(VectorList *head, Vector2 tile) {
    ...
    if (head->previous) {
        vector = (VectorList *) malloc(sizeof(VectorList));
        if (!vector) {
            fprintf(stderr, "Error allocating memory for variable \"vector\", in function addToVectorList()!\n");
            exit(1);
        }
        vector->previous = NULL;
        head->previous = vector
    } else {
        vector = head->previous;
    }
...
Solution solve(Board *board) {
    ...
    while (1) {
        ...
        if (current->next) {
            ...
            current->next->clusters = (Vector2) {-1, -1};
        }
