...
int sort(Item arr[], int l, int r, int (*less) (Item, Item))
{
    int i, j, counter = 0;
    Item pivot, tmp;
    ...
    counter += sort(arr, l, i - 1, less); /* Left side of pivot */
    counter += sort(arr, i + 1, r, less); /* Right side of pivot */
...
int main(int argc, char **argv)
{
    ...
    int numWords;
    /* number of distint words in input file */
    ...
    if ((criterio == alphabetic) && (sentido == ascending)) {
        ...
        /*==== TODO ====*/
        /* -- sort(....); -- */
        OP_CNT = sort((void **)wordtab, 0, numWords - 1, &LessAlphabetic);

    } else if ((criterio == length) && (sentido == ascending)) {
        OP_CNT = sort((void **)wordtab, 0, numWords - 1, &LessLength);
    } else if ((criterio == occurrences) && (sentido == ascending)) {
        OP_CNT = sort((void **)wordtab, 0, numWords - 1, &LessNumUses);
    }
    /* other user options */
    /*==== TODO ====*/
    /* ---------------------------------------- */
    ...
    FreeWordArray(&wordtab, TnumWords);
